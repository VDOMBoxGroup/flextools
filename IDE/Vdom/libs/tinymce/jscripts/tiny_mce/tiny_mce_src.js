...
TinyMCE_Engine.prototype = {
	init : function(settings) {
		...
		/*if (tinyMCE.baseURL.indexOf('://') == -1 && tinyMCE.baseURL.charAt(0) != '/') {
			...
			alert('base url 2' + tinyMCE.baseURL)
		}*/
	...
	getButtonHTML : function(id, lang, img, cmd, ui, val) {
		...
		if (tinyMCE.getParam('button_tile_map') && (!tinyMCE.isIE || tinyMCE.isOpera) && (m = this.buttonMap[id]) != null && (tinyMCE.getParam("language") == "en" || img.indexOf('$lang') == -1)) {
			// Tiled button
			x = 0 - (m * 20) == 0 ? '0' : 0 - (m * 20);
			h += '<a id="{$editor_id}_' + id + '" href="#" onclick="' + cmd + 'return false;" onmousedown="return false;" ' + io + ' class="mceTiledButton mceButtonNormal" target="_self">';
		...
		for (key in ar) {
			...
			if (val != null && val !== '') {
				...
				if (val.indexOf('url(') != -1 && tinyMCE.getParam('convert_urls')) {
					...
					if (m.length > 1)
						val = "url('" + tinyMCE.getParam('urlconverter_callback')(m[1], null, true) + "')";
				}
	...
	_urlConverter : function(c, n, v) {
		if (!c.settings.on_save)
			...
			if (!this.urlConverter) {
				this.urlConverter = tinyMCE.convertURL;///fix this.urlConverter = tinyMCE.settings.urlconverter_callback;
