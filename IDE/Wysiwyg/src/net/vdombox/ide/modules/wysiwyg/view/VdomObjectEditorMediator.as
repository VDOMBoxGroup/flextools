	...
	public class VdomObjectEditorMediator extends Mediator implements IMediator
	{
				...
				case StatesProxy.SELECTED_OBJECT_CHANGED:
				{
					...
					if ( editor.state.substr( 0, 7 ) == "wysiwyg" )
					{
						...
						if ( selectedObject )
						{
							...
							if ( rendererBase && rendererBase.renderVO && rendererBase.renderVO.vdomObjectVO && rendererBase.renderVO.vdomObjectVO.id == selectedObject.id )
		...
		private function drawLine( body : Object ) : void
		{
			...
			for each ( lineVO in listLines )
			{
				...
				{
					if ( element is TransformMarker )
						if ( !lineVO.orientationH && !needDrawVLine || lineVO.orientationH && !needDrawHLine )
							continue;
		...
		private function keyDownDeleteHandler( event : KeyboardEvent ) : void
		{
			...
			if ( multiSelectRenderers )
			{
				...
				Alert.Show( ResourceManager.getInstance().getString( 'Wysiwyg_General', 'delete_Objects' ), ResourceManager.getInstance().getString( 'Wysiwyg_General', 'delete_Renderer' ) + arr.length.toString() + " " + ResourceManager.getInstance().getString( 'Wysiwyg_General', 'elements' ) + " ?", AlertButton.OK_No, component.parentApplication, closeHandler );
			}
			else if ( statesProxy.selectedObject != null )
			{
				...
				Alert.Show( ResourceManager.getInstance().getString( 'Wysiwyg_General', 'delete_Object' ), ResourceManager.getInstance().getString( 'Wysiwyg_General', 'delete_Renderer' ) + componentName + " ?", AlertButton.OK_No, component.parentApplication, closeHandler );
			}
