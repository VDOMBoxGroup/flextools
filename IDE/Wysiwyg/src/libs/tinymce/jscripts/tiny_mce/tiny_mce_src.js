...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			if (!/^[\w-]*:?\/\//.test(u)) {
				base_url = "app:/modules/wysiwyg/libs/tinymce/jscripts/tiny_mce/";//s.base_uri ? s.base_uri.path : new tinymce.util.URI(location.href).directory;
...
(function(tinymce) {
	...
	tinymce.documentBaseURL = "app:/modules/wysiwyg/libs/tinymce/jscripts/tiny_mce/";
	tinymce.baseURL = tinymce.documentBaseURL;	//new tinymce.util.URI(tinymce.documentBaseURL).toAbsolute(tinymce.baseURL);
...
(function(tinymce) {
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			'HiliteColor' : function(command) {
				var value = 0, parent;

				if (parent = dom.getParent(selection.getNode(), 'span')) {
					if (command == 'hilitecolor')
					{
						value = parent.style.backgroundColor;
					}	
				}
		...
		addCommands({
			'FormatBlock' : function(command) {
				var value = 0, nodeObject;

				if (command == 'formatblock')
				{
					nodeObject = dom.getParent(selection.getNode(), 'h1,h2,h3,h4,h5,h6,p,pre,div,address,blockquote,code,dt,dd,samp');  
					if (nodeObject)
						value = nodeObject.nodeName;
				}
