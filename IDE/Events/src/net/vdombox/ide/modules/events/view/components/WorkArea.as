	...
	public class WorkArea extends SkinnableContainer
	{
		...
		private function deleteActionElement( actionElement : ActionElement ) : void
		{
			...
			if ( actionVO is ClientActionVO )
			{
				...
				{
					if ( clientActions[ i ].name == actionVO.name && clientActions[ i ].objectID == actionVO.objectID && clientActions[ i ].id == actionVO.id )
			...
			for ( i = 0; i < events.length; i++ )
			{
				...
				{
					if ( NameUtil.getUnqualifiedClassName( actions[ j ] ) == NameUtil.getUnqualifiedClassName( actionVO ) && actions[ j ].name == actionVO.name && actions[ j ].objectID == actionVO.objectID && actions[ j ].id == actionVO.id )
						actions.splice( j, 1 );
				}
			}
			...
			if ( eventVO && actionVO )
			{
				for each ( eventObject in applicationEventsVO.events )
				{
					if ( eventObject.eventVO == eventVO )
					{
						...
						{
							if ( NameUtil.getUnqualifiedClassName( aviableActionVO ) == NameUtil.getUnqualifiedClassName( actionVO ) && aviableActionVO.name == actionVO.name && aviableActionVO.objectID == actionVO.objectID )
							{
								isAviable = true;
								removeEventListener( MouseEvent.CLICK, mouseClickHandler, true );
								removeShadowHandlers();
		...
		private function element_deleteHandler( event : ElementEvent ) : void
		{
			/*
			   ...
			   Alert.Show( resourceManager.getString( 'Wysiwyg_General', 'delete_Renderer' ) + strName + "
			   ?",AlertButton.OK_No, undoButton.parentApplication, deleteHandler);
