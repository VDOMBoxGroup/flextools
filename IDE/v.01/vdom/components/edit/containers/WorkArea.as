...
//			_contentToolbar.close();
	...
	{	
		if( flag )
		{
			...
			addEventListener(
				MouseEvent.MOUSE_WHEEL, mouseWheelHandler, true );
			
			addEventListener( MouseEvent.ROLL_OUT, rollOutHandler );
			
			renderManager.addEventListener(
				RenderManagerEvent.RENDER_COMPLETE,
				renderManager_renderCompleteHandler );
			
			resizeManager.addEventListener(
				ResizeManagerEvent.OBJECT_SELECT,
				resizeManager_objectSelectHandler );
			
			resizeManager.addEventListener(
				ResizeManagerEvent.RESIZE_COMPLETE,
		...
		else
		{
			...
			resizeManager.removeEventListener(
				ResizeManagerEvent.OBJECT_SELECT,
				resizeManager_objectSelectHandler );
	...
	private function resizeManager_resizeCompleteHandler( event : ResizeManagerEvent ) : void
	{
		...
		for( var name : String in currentProperties )
		{
			if(
				name == "width" && 
				objectDescription.Attributes.Attribute.(@Name == name )[0] ||
				name == "height" && 
				objectDescription.Attributes.Attribute.(@Name == name )[0]
			)
			{
		...
		if(
			changeFlag && IItem( currentObject ).editableAttributes[0] &&
			currentToolBar && !currentToolBar.selfChanged
		)
		{
			var attributes : Object = IItem( currentObject ).editableAttributes[0].attributes;
	...
	private function resizeManager_objectSelectHandler( event : ResizeManagerEvent ) : void {
		...
		if( selectedObject && IItem( selectedObject ).editableAttributes[0] &&
			currentToolBar && !currentToolBar.selfChanged )
		{
			
			var attributes : Object = IItem( selectedObject ).editableAttributes[0].attributes;
			var attributeValue : String;
			var xmlCharRegExp : RegExp = /[<>&"]+/;
