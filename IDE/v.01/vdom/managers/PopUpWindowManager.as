...
public class PopUpWindowManager
{
	...
	public function PopUpWindowManager()
	{
		...
		if( nativeWindow )
			nativeWindow.addEventListener( NativeWindowDisplayStateEvent.DISPLAY_STATE_CHANGING, nativeWindow_displayStateChangingHandler );
	}
	
	public function addPopUp( content : UIComponent,
							  title : String,
							  parent : UIComponent,
							  modal : Boolean = true,
	...
	private function fitToContent( window : Window, content : UIComponent ) : void
	{
		...
		var containerWidth : Number = Math.max( content.width, 
						content.getExplicitOrMeasuredWidth() );
		
		var containerHeight : Number = Math.max( content.height,
	...
	{
		var isLockedWindow : Boolean = popUpInfo.some(
	...
	{		
		var nativeWnd : NativeWindow = Application.application.nativeWindow;
		
		window.move( Math.max( nativeWnd.x + nativeWnd.width / 2 - window.width / 2, 0 ),
					 Math.max( nativeWnd.y + nativeWnd.height / 2 - window.height / 2, 0 ) );
	}
	...
	{
		var newArr : Array = popUpInfo.filter( 
			function ( element:*, index:int, arr:Array ) : Boolean
			{
	...
	private function window_windowCompleteHandler( event : AIREvent ) : void 
	{
		...
		handlerDictionary[ puwd.nativeWindow ] = [ { type : Event.ACTIVATE, handler : nativeWindow_activateHandler } ];
