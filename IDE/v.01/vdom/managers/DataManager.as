...
// ----------------------- end close action -----------------------
	...
	public function setApplicationInformation( 
						applicationId : String, attributes : XML ) : void
	{
		soap.set_application_info( applicationId, attributes );
	}
	
	public function getApplicationStructure( 
						applicationId : String = "" ) : void
	{
		if( applicationId == "" )
			applicationId = currentApplicationId;
								...
								applicationId : String = "" ) : void
	{
		...
		if( applicationId )
		{
			soap.create_object( applicationId, parentId, 
								typeId, objectName, attributes );
		}
		else if ( currentApplicationId )
		{
			soap.create_object( currentApplicationId, parentId, 
								typeId, objectName, attributes );
		}
	}
	...
	{
		soap.submit_object_script_presentation( currentApplicationId, 
										currentObjectId, objectXMLScript );
	}
	...
	/**
	 * 
	 * @param selectedObjects описание типа объекта,
	 *  которое надо сохранить, для последующей отправки на сервер.
	 */	
	public function updateAttributes( newObjectId : String = "none", 
										attributes : XML = null ) : void
	{
		var objectId : String;
		...
		else
		{
			...
			var dmEvent : DataManagerEvent = new DataManagerEvent( 
		...
		if( newOnlyAttributes )
		{
			...
			proxy.setAttributes( currentApplicationId, objectId,
								 newOnlyAttributes );
		}
		
		if( nameChanged )
			soap.set_name( currentApplicationId, oldXMLDescription.@ID, 
							newXMLDescription.@Name );
	}
	...
	private function deleteXMLNodes( objectId : String ) : void
	{
		...
		var deleteNodes : XMLList = 
				_currentApplication..Objects.Object.( @ID == objectId );
		
		for ( var i : int = deleteNodes.length() - 1; i >= 0; i-- )
			delete deleteNodes[i];
	}
	
	private function extractNewValues( oldAttributes : XMLList, 
											newAttributes : XMLList ) : XML
	{
	...
	private function soap_setApplicationInfoHandler( event : SOAPEvent ) : void
	{
		...
		var dme1 : DataManagerEvent = new DataManagerEvent( 
	...
	{
		var dme : DataManagerEvent = new DataManagerEvent( 
						DataManagerEvent.GET_APPLICATION_STRUCTURE_COMPLETE );
	...
	{
		var dme : DataManagerEvent = new DataManagerEvent( 
						DataManagerEvent.SET_APPLICATION_STRUCTURE_COMPLETE );
	...
	{	
		var dme : DataManagerEvent = new DataManagerEvent( 
							DataManagerEvent.GET_APPLICATION_EVENTS_COMPLETE )
		dme.result = event.result;
		dispatchEvent( dme );
	}
	...
	{
		var dme : DataManagerEvent = new DataManagerEvent( 
							DataManagerEvent.SET_APPLICATION_EVENTS_COMPLETE )
	...
	private function soap_getAllTypesHandler( event : SOAPEvent ) : void
	{
		...
		dispatchEvent( 
			new DataManagerEvent( DataManagerEvent.LOAD_TYPES_COMPLETE )
		);
	}
	...
	{	
		...
		dispatchEvent( new DataManagerEvent( 
					DataManagerEvent.GET_OBJECT_XML_SCRIPT_COMPLETE, result ));
	}
	...
	private function soap_setObjectScriptPresentationHandler( event : SOAPEvent ) : void
	{
		...
		dispatchEvent( 
			new DataManagerEvent( 
				DataManagerEvent.SET_OBJECT_XML_SCRIPT_COMPLETE, result )
		);
	}
	...
	{
		var dme : DataManagerEvent = new DataManagerEvent( 
										DataManagerEvent.SEARCH_COMPLETE );
	...
	private function faultHandler( event : FaultEvent ) : void
	{
		...
		switch( errorCode )
		{
			...
			{
				dme = new DataManagerErrorEvent( 
										DataManagerErrorEvent.GLOBAL_ERROR );
