...
// ----------------------- end close action -----------------------
		...
		private function soap_getTopObjectsHandler( event : SOAPEvent ) : void
		{
			...
			_currentApplication.appendChild( 
				<Objects/>
				);
			...
			dispatchEvent( new DataManagerEvent( DataManagerEvent.LOAD_APPLICATION_COMPLETE ) );
		}
		...
		{
			var dme : DataManagerEvent = new DataManagerEvent( DataManagerEvent.GET_SERVER_ACTIONS_COMPLETE );
		...
		{
			var dme : DataManagerEvent = new DataManagerEvent( DataManagerEvent.SET_SERVER_ACTIONS_COMPLETE );
