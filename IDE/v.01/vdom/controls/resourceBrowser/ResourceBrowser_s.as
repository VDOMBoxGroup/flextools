...
private function doneHandler() : void
{
	...
	{
		dispatchEvent( new ResourceBrowserEvent( ResourceBrowserEvent.RESOURCE_SELECTED,
													  __thumbsList.selectedItem.@id,
													  __thumbsList.selectedItem.@name ) );
	}
	else
	{
		dispatchEvent( new ResourceBrowserEvent( ResourceBrowserEvent.RESOURCE_CANCELED ) );
	}
}
...
private function closeHandler( cEvent : Event ) : void
{
	registerEvent( false );
}
...
private function uploadButton_clickHandler() : void
{
	...
	fileForUpload.browseForOpen( "Choose file to upload", [ imagesFilter, docFilter,
															allFilesFilter ] );
}
...
private function refreshButton_clickHandler() : void 
{
	listResourcesQuery();
}
