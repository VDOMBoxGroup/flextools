...
//			dispatchEvent(new Event(Event.CHANGE));
			...
			if ( _mark )
			{
				this.graphics.lineStyle( 10, _numColor, .1, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.moveTo( x0, y0 );
				this.graphics.lineTo( x1, y1 );
				...
				this.graphics.lineStyle( 3, _numColor, 1, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.moveTo( x0, y0 );
				this.graphics.lineTo( x1, y1 );

				this.graphics.lineStyle( 3, 0x000000, .4, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.moveTo( x0, y0 );
				this.graphics.lineTo( x1, y1 );

				this.graphics.lineStyle( 1, _numColor, 1, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
			...
			else
			{
				...
				this.graphics.lineStyle( 3, _numColor, 1 * _alpha, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.moveTo( x0, y0 );
				this.graphics.lineTo( x1, y1 );

				this.graphics.lineStyle( 3, 0x000000, .4 * _alpha, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.moveTo( x0, y0 );
				this.graphics.lineTo( x1, y1 );

				this.graphics.lineStyle( 1, _numColor, 1 * _alpha, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				...
				this.graphics.lineStyle( 3, _numColor, _alpha, false, LineScaleMode.NONE, CapsStyle.SQUARE,
										 JointStyle.MITER );
				this.graphics.lineTo( x1 + Math.cos( alf + dA ) * dDist, y1 + Math.sin( alf + dA ) * dDist );
				this.graphics.lineTo( x1 + Math.cos( alf - dA ) * dDist, y1 + Math.sin( alf - dA ) * dDist );
				this.graphics.lineTo( x1, y1 );
		...
		private function calculatePoints() : void
		{
			...
			if ( fromObj )
			{
				...
				_pFromObj = new Point( fromObj.x + fromObjHalfWidth, fromObj.y + fromObjHalfHeight );
			}
			...
			if ( toObj )
			{
				...
				if ( toObjWidth )
				{
					...
					_pToObj = new Point( toObj.x + toObjHalfWidth, toObj.y + toObjHalfHeight );
				}
				else
				{
					_pToObj = new Point( toObj.x, toObj.y );
				}
			}
