package net.vdombox.ide.modules.tree.view
{
	...
	public class TreeCanvasMediator extends Mediator implements IMediator
	{
		...
		private function mouseUpHandler( event : MouseEvent ) : void
		{
			...
			var targetTreeElement : TreeElement = getTreeElementFromParent( event.target as DisplayObjectContainer );

			treeCanvas.linkagesContainer.removeElement( shadowLinkage );

			if ( shadowLinkage && targetTreeElement )
			{
				shadowLinkageVO.target = targetTreeElement.treeElementVO;
				sendNotification( ApplicationFacade.CREATE_LINKAGE, shadowLinkageVO );
			}
		...
		private function mouseMoveHandler( event : MouseEvent ) : void
		{
			...
			if ( viewport.mouseY > visibleHeight - 100 && viewport.verticalScrollPosition < treeCanvas.scroller.verticalScrollBar.maximum )
