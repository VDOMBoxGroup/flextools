...
package ro.victordramba.asparser
{
	...
	internal class Tokenizer
	{
		...
		private static const keywordsA:Array = [
			'as', 'break', 'case', 'continue', 'default', 'do', 'while', 'else', 'for', 'in', 'each',
		...
		private static const symbolsA:Array = [
			...
			'as', ',', '?', 'delete', '.', 'in', 'instanceof', 'is', '::', 'new', '{', '}',
		...
		public function Tokenizer(str:String, typeDB:TypeDB)
		{
			this.typeDB = new TypeDB;
			this.typeDB.parentDB = typeDB;
		...
		public function nextToken():Token
		{
			...
			if (isLetter(c))
			{
				...
				if (isKeyword(str))
				    ...
				    type = Token.KEYWORD2;
				else if (tokens.length && tokens[tokens.length-1].string == '[' && (str == 'Embed' || str=='Event'))
					type = Token.KEYWORD;
				else
		...
		internal function runSlice():Boolean
		{
				...
				tp.string=='var' || tp.string=='const'))
			{
				...
				if (_scope && tp.string=='class' || tp.string=='interface' || scope.fieldType=='package')
				{
					...
					try {
						typeDB.addDefinition(scope.name, field);
					}
