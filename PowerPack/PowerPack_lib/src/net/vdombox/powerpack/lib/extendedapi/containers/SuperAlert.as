...
public class SuperAlert extends SuperWindow
{
	...
	{
		var cornerRadius : Number = panel.getStyle("cornerRadius");
		var gradientColors : Array = panel.getStyle("backgroundGradientColors");
		
		var borderThickness : Number = panel.getStyle("borderThickness");
		var borderColor : int = panel.getStyle("borderColor");
		
		panel.graphics.clear();
		graphics.clear();
		
		drawRoundRect(0, 0, width, height, cornerRadius, borderColor, 1);
		
		var topRectX : Number = borderThickness;
		var topRectY : Number = borderThickness;
		var topRectW : Number = width - 2*borderThickness;
		var topRectH : Number = height - 2*borderThickness;
