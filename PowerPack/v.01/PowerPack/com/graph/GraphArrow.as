package PowerPack.com.graph
{
	...
	[Event(name="arrowLanguageChanged", type="flash.events.Event")]		
	...
	public class GraphArrow extends UIComponent implements IFocusManagerComponent
	{
	    ...
	    */
	    ...
	    private static var alertDeleteText:String = "Are you sure want to delete this arrow?";
	    ...
	    public static function set langXML(value:XML):void
	    {
			...
			alertDeleteTitle = _langXML.grapharrow.alertdeletetitle;
			alertDeleteText = _langXML.grapharrow.alertdeletetext;

        	menuItemCaptions = [];

	    	menuItemCaptions = new Array(
	    		_langXML.grapharrow.menuitemselecttrans, 
	    		_langXML.grapharrow.menuitemdelete, 
    			_langXML.grapharrow.menuitemenable, 
    			_langXML.grapharrow.menuitemhighlight );	   
	        	        
	        Application.application.dispatchEvent(new Event("arrowLanguageChanged"));
	    }
		...
		public function GraphArrow()
		{
			...
			Application.application.addEventListener("arrowLanguageChanged", languageChangeHandler);
		...
		{			
			...
			Application.application.removeEventListener("arrowLanguageChanged", languageChangeHandler);
            removeEventListener(FlexEvent.CREATION_COMPLETE, createCompleteHandler);

           	removeEventListener(FocusEvent.FOCUS_IN, arrowFocusInHandler);	
           	removeEventListener(FocusEvent.FOCUS_OUT, arrowFocusOutHandler);
            removeEventListener(KeyboardEvent.KEY_DOWN, arrowKeyDownHandler);            
	    ...
	    override protected function commitProperties():void
	    {
	        ...
	        if (bToObjChanged)
	        {
	    		...
	    		{
					if(ArrayUtil.getItemIndex(data, GraphNode(fromObject).arrTrans)<0)
	    			{
	    ...
	    {	        
	        systemManager.addEventListener(
	            MouseEvent.MOUSE_MOVE, arrow_systemManager_mouseMoveHandler, true);
	
	        systemManager.addEventListener(
	            MouseEvent.MOUSE_DOWN, arrow_systemManager_mouseDownHandler);
	    }
	    ...
	    {	    
			focusManager = Container(parent).focusManager;           	
           	...
           	addEventListener(FocusEvent.FOCUS_IN, arrowFocusInHandler);	
           	addEventListener(FocusEvent.FOCUS_OUT, arrowFocusOutHandler);
	    	addEventListener(KeyboardEvent.KEY_DOWN, arrowKeyDownHandler);
	    }
	    ...
	    private function arrow_systemManager_mouseMoveHandler(event:MouseEvent):void
	    {
	    	bChanged = true;
	    ...
	    private function arrow_systemManager_mouseDownHandler(event:MouseEvent):void
	    {
	    	if(!toObj)
	    		destroy();
	    }	
		private function arrowKeyDownHandler(event:KeyboardEvent):void
	    {	    	
	    	if(!fromObj)
	    		return;
	    	// delete
		...
		private function arrowFocusInHandler(event:FocusEvent):void
		{
			bChanged = true;
			invalidateDisplayList();	
		}			
		private function arrowFocusOutHandler(event:FocusEvent):void
		{
			bChanged = true;
			invalidateDisplayList();
		}
