...
package com.yahoo.astra.mx.controls.colorPickerClasses
{
	...
	//--------------------------------------
		...
		/**
		 * @copy com.yahoo.astra.mx.controls.colorPickerClasses.IColorPicker#selectedColor
		 */
	...
	//--------------------------------------
		...
		protected function positionSelectionFromColor(color:HSBColor):void
		{
			...
			if(colorFound)
			{
				...
				{
					position = PointUtil.polarToXY(degrees * Math.PI / 180, distance + outerColorSize / 2).add(center);
				}
				else if(color.touint() != 0xffffff)
				{
					position = PointUtil.polarToXY(degrees * Math.PI / 180, distance).add(center);
				}
			}
		...
		protected function getColorFromPosition(position:Point):HSBColor
		{
			...
			var distance:Number = PointUtil.distance(position, center);
