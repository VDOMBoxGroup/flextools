...
private function registerEvent( flag : Boolean ) : void
{
	if ( flag )
	{
		...
		searchResult.addEventListener( SearchResultEvent.SEARCH_OBJECT_SELECTED,
									   searchResult_searchHandler );
	...
	else
	{
		...
		searchResult.removeEventListener( SearchResultEvent.SEARCH_OBJECT_SELECTED,
										  searchResult_searchHandler );
...
private function registerWatchers( flag : Boolean ) : void
{
	if ( flag )
	{
		watchers.push( BindingUtils.bindProperty( searchPanel, "listApplication",
												  dataManager, "listApplication" ),
					   BindingUtils.bindProperty( searchPanel, "applicationId", dataManager,
												  "currentApplicationId" ), BindingUtils.bindProperty( listApplicationContainer,
																									   "dataProvider",
																									   dataManager,
																									   "listApplication" ),
					   BindingUtils.bindProperty( listApplicationContainer, "applicationId",
												  dataManager, "currentApplicationId" ),
					   BindingUtils.bindProperty( applicationInformation, "dataProvider",
												  dataManager, "currentApplicationInformation" ) );
	}
	else
	{
...
private function searchResult_searchHandler( event : SearchResultEvent ) : void
{
	...
	dataManager.changeCurrentApplication( applicationId );
...
private function dataManager_pageChanged( event : DataManagerEvent ) : void
{
	...
	if ( dataManager.currentObjectId == objectId )
	{
